...
/**
 * Insert a new class interest registration
 */
...
/**
 * Get all registrations for a specific class type
 */
export async function getRegistrationsByClassType(classType: string) {
  const db = await getDb();
  if (!db) {
  ...
  const result = await db.select().from(registrations).where(eq(registrations.classType, classType));
  return result;
}
